...
import type { TierEntry, TierRole, TierRank } from "@/types";

type Role = "All" | TierRole;

// Static fallback — shown while loading or if the API is unreachable
const STATIC_TIER_LIST: TierEntry[] = [
...
const TIER_CONFIG: Record<TierRank, { label: string; bg: string; text: string; border: string }> = {
...
export default function TierListPage() {
  ...
  const [expandedTiers, setExpandedTiers] = useState<Set<TierRank>>(new Set(["S", "A", "B"]));
  ...
  const entries: TierEntry[] = data?.entries ?? STATIC_TIER_LIST;
  const patch: string = data?.patch ?? "—";

  const toggleTier = (tier: TierRank) => {
  ...
  const filtered = entries.filter(
    (e) => filterRole === "All" || e.roles.includes(filterRole as TierRole)
  );
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6 animate-fade-in">
      ...
      <div className="space-y-2">
        {TIERS.map((tier) => {
          const tierEntries = filtered.filter((e) => e.tier === tier);
            ...
            <div key={tier} className={cn("overflow-hidden rounded-lg border", cfg.border)}>
              ...
              >
                ...
                <div className="flex flex-1 flex-wrap gap-1.5">
                  {tierEntries.slice(0, expanded ? undefined : 6).map((e) => (
                    <div
                      key={e.name}
